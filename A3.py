...
class User:
    ...
    def withdraw(self, amount):
        if amount <= self.balance and self.balance - amount > 5000:
            self.balance -= amount
            self.bank.users[self.username]["balance"] = self.balance
            self.bank.save_data()
            return amount
        else:
            print("Insufficient balance or below minimum required balance.")
            return 0
    ...
    def login(self, username, password, role):
        if role == "user" and username in self.users:
            if self.login_attempts.get(username, 0) >= 3:
                ...
                print("Incorrect password. Please try again(Note:Minimum attempts only).")
                self.login_attempts[username] = self.login_attempts.get(
                    username, 0) + 1
                if self.login_attempts[username] == 3:
                    print(
                        f"Too many unsuccessful login attempts for user {username}.\n account locked,Contact admin")
                return False
